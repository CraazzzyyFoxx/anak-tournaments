  ...
  return (
    <Card className="relative bg-cover bg-center rounded-lg shadow-md overflow-hidden transition-transform transform hover:scale-110 min-h-[240px] group aspect-square">
      <Image
        ...
        quality={100}
      />
      <div className="absolute inset-0 bg-background/20 transition duration-300 group-hover:bg-opacity-60" />
      <div className="absolute top-2 right-2 bg-background/50 text-white text-xs rounded px-2 py-1 z-10">
          {(achievement.rarity * 100).toFixed(2)}%
      </div>
      <div className="absolute bottom-2 right-2 bg-background/50 text-white text-xs rounded px-2 py-1 z-10">
        x{achievement.count}
      </div>
      ...
      <div className="absolute bottom-2 left-2  bg-opacity-0 text-white transition-opacity duration-300 opacity-0 group-hover:opacity-100 z-10">
