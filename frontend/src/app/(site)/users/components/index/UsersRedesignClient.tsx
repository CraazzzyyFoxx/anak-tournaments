...
const UsersRedesignClient = () => {
  ...
  const order = parseOrder(searchParams?.get("order") ?? null);
  const view = parseView(searchParams?.get("view") ?? null);
  const role = (searchParams?.get("role") as UserRoleType | null) ?? undefined;
  const divMin = clampDivisionToGrid(divisionGrid, parseOptionalInt(searchParams?.get("div_min") ?? null));
  const divMax = clampDivisionToGrid(divisionGrid, parseOptionalInt(searchParams?.get("div_max") ?? null));
  ...
  // `useQueryParams` already resets `page` whenever any other param changes, so
  // the old `keepPage` flag is gone: paging passes only `page` and is left alone.
  ...
  const handleRoleChange = useCallback(
    (value: "all" | UserRoleType) => {
      setParams({ role: value === "all" ? undefined : value });
    },
    [setParams]
  );
  ...
  const handleViewChange = useCallback(
    (next: ViewMode) => {
      setParams({ view: next === "analytics" ? undefined : next });
    },
    [setParams]
  );
  ...
  const handleLetterChange = useCallback(
    (value: string | null) => {
      setParams({ letter: value === null ? undefined : value });
    },
    [setParams]
  );
