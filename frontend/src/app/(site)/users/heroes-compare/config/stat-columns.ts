...
export const COL: Record<string, StatColumnDef> = {
  ...
  // "Damage taken" is the defensive/tank-flavoured metric, so it maps onto the
  // tank role token. The `/65` bar tint is spelled out as an explicit color-mix
  // rather than a Tailwind opacity modifier so the class survives as a literal.
  per10_damage_taken:      { key: "per10_damage_taken",      labelKey: "users.heroesCompare.stat.dmgTaken",   ascending: true,  formatValue: _fmt.large, barColor: "bg-[color:color-mix(in_srgb,var(--aqt-tank)_65%,transparent)]", accentColor: "bg-[color:var(--aqt-tank)]", getValue: _k("per10_damage_taken") },
