...
interface PlatformConfig {
  ...
  updateFn: (userId: number, identityId: number, data: Record<string, string>) => Promise<User>;
...
const PLATFORMS: PlatformConfig[] = [
  {
    ...
    addFn: (userId, data) => adminService.addDiscordIdentity(userId, data as { name: string }),
    updateFn: (userId, id, data) =>
      adminService.updateDiscordIdentity(userId, id, data as { name: string }),
  ...
  },
  {
    ...
    addFn: (userId, data) =>
      adminService.addBattleTagIdentity(userId, data as { battle_tag: string }),
    updateFn: (userId, id, data) =>
      adminService.updateBattleTagIdentity(userId, id, data as { battle_tag: string }),
  ...
  },
  {
    ...
    getAddPayload: (value) => ({ name: value }),
    getUpdatePayload: (value) => ({ name: value }),
    addFn: (userId, data) => adminService.addTwitchIdentity(userId, data as { name: string }),
    updateFn: (userId, id, data) =>
      adminService.updateTwitchIdentity(userId, id, data as { name: string }),
...
function IdentityRow({
  ...
  const updateMutation = useMutation({
    mutationFn: () =>
      platform.updateFn(userId, identity.id, platform.getUpdatePayload(editValue)),
...
function AddIdentityRow({ platform, userId, onUserUpdated }: AddIdentityRowProps) {
  const queryClient = useQueryClient();
  const [value, setValue] = useState("");
  const inputRef = useRef<HTMLInputElement>(null);

  const addMutation = useMutation({
    mutationFn: () => platform.addFn(userId, platform.getAddPayload(value.trim())),
    onSuccess: (updatedUser: User) => {
      queryClient.invalidateQueries({ queryKey: ["admin", "users"] });
      onUserUpdated(updatedUser);
      setValue("");
  ...
  return (
    <div className="space-y-1.5">
      ...
      {addMutation.isError && addMutation.error instanceof Error && (
        <p className="text-xs text-destructive px-1">{addMutation.error.message}</p>
      )}
    </div>
  );
}
...
function PlatformTab({
  ...
  return (
    <div className="space-y-3">
      ...
      {canEdit && (
        <AddIdentityRow platform={platform} userId={userId} onUserUpdated={onUserUpdated} />
      )}
    </div>
  );
}
...
function PlatformTabTrigger({ platform, count }: PlatformTabTriggerProps) {
  return (
    <TabsTrigger value={platform.key} className="gap-1.5 data-[state=active]:gap-1.5">
...
export function IdentityManagement({
  ...
  const handleIdentityDeleted = (platformKey: string, identityId: number) => {
    setUser((prev) => ({
      ...prev,
      [platformKey]: (prev[platformKey as keyof User] as IdentityItem[]).filter(
  ...
  const totalCount = Object.values(itemsMap).reduce((sum, arr) => sum + arr.length, 0);

  // Default to the tab with the most identities, or discord
  const [defaultTab] = useState(() =>
    PLATFORMS.reduce((best, p) =>
      (itemsMap[p.key]?.length ?? 0) > (itemsMap[best.key]?.length ?? 0) ? p : best,
    ).key,
  );
