...
function MatchReportDialogBody({ encounter, onOpenChange }: Omit<MatchReportDialogProps, "open">) {
  ...
  // dialog never grows fields under the captain's cursor.
    ...
    mutationFn: () =>
      captainService.submitReport(encounter.id, {
        ...
        map_codes: mapCodesConfig.enabled
          ...
          : [],
        comment:
          commentConfig.enabled && comment.trim().length > 0 ? comment.trim() : null,
  ...
  return (
    <DialogContent className="max-w-lg">
          ...
          <div className="space-y-3 rounded-xl border border-[color:var(--aqt-border-2)] bg-[color:var(--aqt-overlay-2)] p-4">
            ...
            <div className="space-y-2">
              {slots.map((slot, position) => {
                ...
                const isMissing =
                  mapCodesConfig.required && isPlayed && value.trim().length === 0;
      ...
      {/* The per-field errors live inside the scroll area above; the footer does
          not. Repeating the first blocking reason here keeps the greyed-out
          submit from ever being unexplained on screen. */}
      <DialogFooter className="mt-6 flex flex-row flex-wrap items-center justify-end gap-x-4 gap-y-2">
        {validationError ? (
          <p
            role="status"
            className="mr-auto text-xs font-semibold text-danger"
          >
            {validationError}
          </p>
        ) : null}
