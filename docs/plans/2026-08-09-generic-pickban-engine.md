...
- **Undo by consent (post-implementation addition).** A misclicked action used to cost the whole round: the only remedy was an organizer's `reset_pick_ban_session`. Captains now have a surgical one — `POST .../pick-ban/{kind}/undo` records one side's consent, and the OPPONENT's matching call applies it (`pick_ban_undo.perform_undo`), because the last action is information the other side has already played around. Two columns carry it (`pick_ban_session.undo_requested_by` / `undo_target_index`), and the target index is what keeps a consent from ever applying to an action that landed after it — a new action clears the request outright. What is undoable is `pick_ban_engine.undoable_entries`: the newest committed entry plus any `decider` the engine resolved off the back of it (undoing that alone would be a no-op the next read reverses), refused once the round has been `played` or a later round has opened. A revert restores `available`, deletes the ban's ledger row, reopens a `completed` session and restarts the turn clock — without that last part `auto_resolve_timeout` would re-take the action at random on the next read. A MAP undo additionally waits on that round's hero bans, which opened off the pick and are never withdrawn.
