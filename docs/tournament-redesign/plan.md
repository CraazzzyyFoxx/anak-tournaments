...
- Обложка: `coverUrl={tournament.cover_image_url}` в `PageHero`. В `HeroFrame` — размытая цветовая заливка на всю площадь, под сеткой и свечением: `filter: blur(28px) saturate(2.2)`, `mix-blend-mode: color`, `inset: -96px` (вылет за рамку ≥3 радиусов блюра, иначе блюр затягивает прозрачность извне бокса и оттенок гаснет у верхнего и нижнего края — `scale(1.12)` давал 12px запаса вместо нужных ~84 и виньетку 24→38.6→23 по пикселям). Всё **инлайном**, а не Tailwind-классами: `mix-blend-*` и `saturate-*` в этой сборке не сгенерированы, и класс молча не применился — первый рендер вышел 70%-непрозрачным фото. Blend берёт тон из картинки, а яркость из рамки, поэтому контраст не зависит от загрузки; скрим этого не может — `--aqt-fg-faint` и так на пороге AA. Нет обложки → рамка как раньше, ничего не резервируется. §2 ⑦.
- Постер: обложка **целиком** в правой колонке шапки — фикс-бокс `284×160` + `object-contain`, размеры **инлайном** (`max-h-40` в этой сборке не сгенерировался и картинка вылезла в 1920px; авто-размер к тому же брал грид-трек от родной ширины и сжимал h1). Реальная обложка — 1920×1080 постер с названием турнира внутри картинки, поэтому любой кроп под форму шапки уничтожает её содержимое. CTA и «Драфт →» переехали из `aside` в `actions` (под чипами). Высота шапки: 1280 → ~200px; 375 → 429px с постером против 253px без.
...
- §3A: `PhaseTimeline horizontal` первым блоком (`id="phases"`); прогресс регистрации по ролям — из `registrations_count` и ролей регистраций (запрос участников уже есть у `TournamentParticipantsPage` — переиспользовать его query key, не заводить второй); для `team_formation === "registration"` — «команд N / M». Формат — стадии + `roster_shape`. `MapPool tiles` (`id="map-pool"`). Ссылки — `TournamentLinkChips`.
...
- **Справочный хвост — во всех трёх ветках** (§3 ⑨⑩), потому что шапка его больше не несёт: карточка «Формат» (строка «Стадии» + team formation с глифами `roster_shape` + полное `description`) и последней — «Ссылки» (`TournamentLinkChips`). Строка «Стадии» — имена стадий организатора в порядке `order`, у каждой её тип в скобках через реестр `STAGE_TYPE_LABEL` (ключи `common.roundRobin`, `common.swiss`, `bracket.*Elimination`). Производный `formatLabel` **удалён** вместе с ключами `common.formatLabel.*`: под заголовком «Формат» он давал вторую строку «Формат» и повторял те же стадии — «Groups → Playoff — Groups → Playoffs». В §3A «Формат» остаётся в левой колонке под прогрессом регистрации (иначе левая колонка — одна карточка), в §3B/§3C — в правой перед «Ссылками». Признак «рисовать ли заголовок карточки ссылок» — `visibleTournamentLinks(links).length > 0`, экспортируемый из `TournamentLinkChips`: реестр чипов один, и турнир с одной только стрим-ссылкой карточку не получает. В §3A правая колонка теперь появляется и ради одних ссылок (`hasAside`), не только ради мап-пула.
