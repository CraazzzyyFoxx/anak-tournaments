...
### 2026-09-03 · P2 · пробелы кита, найденные потребителями

- `EntityHubHeader` рендерит `<h1>`, а хаб уже отдал один на турнир. Заголовку
  стадии в `bracket/StageEditor` нужен `<h2>` в том же визуальном регистре —
  сделан вручную. Киту не хватает пропа уровня заголовка.
- `SaveBar` жёстко включает `useUnsavedGuard({ guardNavigation: true })`, поэтому
  переключение `?section=` внутри редактора стадии при dirty спрашивает
  «Discard unsaved changes?», хотя компонент не размонтируется. `EntityFormDialog`
  этот флаг наружу отдаёт, `SaveBar` — нет.
- `WizardShell` рендерит рельс шагов как индикатор. Клик по шагу назад
  (`canNavigateToWizardStep`, `canNavigateToSetupStep`) стал мёртвым кодом и
  удалён вместе с ним; назад — только кнопкой Back.

Кит не форкался ни в одном из трёх случаев. Правки — в P6 либо отдельным PR.

### 2026-09-03 · P2-8 · у визарда появились `?step=` и `?source=`

§1.2 таких параметров не перечисляет, но DoD п.3 требует состояние в URL, а до
этого WU шаг жил в `useState` — сохранять было нечего. Введены `step` и `source`
(вход «Import from Challonge»). Заодно у роута появился гейт `tournament.create`,
которого не было вообще: кнопку в списке право скрывало, а вставленный URL — нет.

### 2026-09-03 · P2-3 · Entries: два пробела в данных и одно изменение поведения

- Чипа `division` нет: `AdminRegistrationRole` несёт `rank_value`, не дивизион,
  а вывод дивизиона требует грида воркспейса — запроса, который
  `TournamentHubShell` сознательно не делает на каждой странице хаба. Нужен
  либо дивизион в ответе регистраций, либо решение про грид-запрос.
...
`AdminInspector` сам решает panel-vs-sheet, поэтому файл с именем `*Sheet`,
который никакого шита не экспортирует, называл бы себя неправильно. Ассерты
теста перенесены без изменений.

### 2026-09-03 · P2-6 · активная стадия больше не открывается сама

T4 требует «нет `?stage=` → нет выбора», иначе на узком экране список никогда
не показывается, а `emptyDetail` недостижим. Прежний экран сам открывал
активную (или первую) стадию. Плюс в General появилось поле Name — переименование
стадии, которого на прежнем экране не было (F7 его показывает).

### 2026-09-03 · P2 · браузерная проверка (DoD п.6) не выполнена

Ни у меня, ни у субагентов не получилось открыть авторизованную админку:
`/api/auth/me` отдаёт 401, стаб через перехват запросов работает нестабильно
(приложение зависает на `LoadingState`, не запрашивая `/me`), а `omp browser
relay` не подключён — расширения в Chrome нет. Вместо этого по каждому WU:
`typecheck`, `lint:design`, полный `vitest` (1408 зелёных), curl роутов на
dev-сервере и behavior-тесты, ассертящие реальный DOM. Проверку на
375/768/1280/1440 нужно догнать одним проходом, когда появится сессия.

### 2026-09-03 · P6 · `meta.filter` остаётся после удаления воронок

§9 просит удалить `AdminColumnFilter.tsx` и header-funnel код — сделано. Но
сама декларация `meta.filter` на колонке удалена только там, где она была
нужна воронке (`content/heroes`, `content/maps` — их фильтры стали чипами
`AdminFilterBar`, а запрос читает значение из `useAdminFilters`). В
`balancerRegistrationColumns` (экран Registration › Entries) она сохранена:
в client-mode это единственный способ связать URL-параметр с колонкой, чей
`filterFn` его применяет, — то есть контракт «параметр ↔ колонка», а не
контрол. Чипы на этом экране уже были (P2-3), поменялось только то, что
воронок в шапке больше нет.

Заодно из `AdminColumnFilterSpec` убраны `label` и `searchable`, а из
`AdminColumnFilterOption` — `count`: это была презентация воронки (её
aria-label, её поисковая строка, её счётчики), и после удаления контрола их
никто не читает. `toggleFilterValue` удалён по той же причине — единственным
вызывающим была воронка; три его теста удалены вместе с ним.

### 2026-09-03 · P6 · `nextTournamentFilterQuery` удалён вместе с `TournamentFilterSelect`

§9 просит оставить «`TOURNAMENT_QUERY_PARAM`/парсеры». Оставлены
`TOURNAMENT_QUERY_PARAM` и `parseTournamentQueryParam` — их читают шесть
экранов. Третий экспорт, `nextTournamentFilterQuery`, строил query-string для
`onValueChange` того самого `<Select>` и после его удаления не имел ни одного
вызывающего: `useAdminFilters` пишет URL сам. Файл стал чистым модулем без
JSX, поэтому с него снят и `"use client"`.

### 2026-09-03 · P6 · `AdminDetailTable` оставлен

§9 разрешает оставить его «с комментарием», если `TeamRosterEditor` не
переехал. Оставлен, и вот почему он не должен переезжать: оба потребителя
(`teams/TeamRosterEditor`, `TournamentTeamsTab`) — не браузеры. У них нет
поиска, пагинации, сортировки, URL-состояния и колонки действий, а строка —
инлайн-редактор, а не ссылка на него. `AdminDataTable` добавил бы им тулбар,
пейджер и URL-контракт, причём на экране, где настоящая таблица уже есть, два
экземпляра конкурировали бы за `?page=`/`?sort=`. Общим у этих двух гридов
был не движок, а набор классов — этим файл и остался; причина записана в его
шапке.

### 2026-09-03 · P6 · `SaveBar.guardNavigation`: закрыт пробел, остался компромисс

Пробел из записи «2026-09-03 · P2» закрыт так же, как он выглядит в
`EntityFormDialog` — булев проп с дефолтом `true`. Поставлен в `false` на двух
экранах, чья роутовая под-навигация является частью той же формы: редактор
стадии (`?section=`, табы — ссылки) и редактор черновика дивизионов
(`?tab=`, куда pre-flight «Ready to publish?» и отправляет разбираться с
маппингами).

Компромисс: проп выключает перехват *всех* якорей, а не только внутренних,
поэтому на этих двух экранах уход по настоящей ссылке (сайдбар, табы хаба,
`encountersHref`) больше не спрашивает про несохранённое. Половина
`beforeunload` остаётся включённой, так что перезагрузка и закрытие вкладки
по-прежнему перехватываются. Точный вариант («внутри формы» как предикат, а
не флаг) — это уже другой API кита, а не закрытие пробела; если ложные
уходы окажутся заметны, менять надо `useUnsavedGuard`, а не экраны. Список
стадий, кстати, гарду и не был виден никогда: его строки — `<button>`, не
ссылки.

### 2026-09-03 · P6 · `EntityHubHeader.level`

Второй пробел из той же записи закрыт: `level?: 1 | 2` задаёт и ранг
заголовка, и его размер (`text-2xl` / `text-lg`). `bracket/StageEditor`
переведён на кит и потерял свою ручную шапку вместе с `Badge`, `TONE_CLASS` и
`cn`. Тест на ранг заголовка — и в `EntityHubHeader.behavior.test.tsx`, и в
`bracket/page.behavior.test.tsx` (на странице ровно один `<h1>`, и он
принадлежит хабу).

### 2026-09-03 · P6 · Publish/Activate остаются в шапке Divisions, а не в `SaveBar`

Третий пункт P6 предлагал дать `SaveBar` способ показывать постоянные
действия при чистой форме и вернуть в него Publish/Activate. Не сделано,
решение — шапка правильный дом:

1. `publishChecks` содержит блокирующую проверку `saved: ok: !dirty`
   (`editor/publishChecks.ts`), то есть `publishable === false` ровно тогда,
   когда `SaveBar` виден. Publish внутри `SaveBar` был бы всегда disabled.
2. Publish и Activate — переходы жизненного цикла версии, а не сохранение
   формы: по §4 T3 это `EntityHubHeader.actions`, где уже стоят Undo и
   Discard draft, и где в хабе турнира стоит `TournamentStatusControl`.
3. `SaveBar` — это `role="region" aria-label="Unsaved changes"`. Отрисовка
   этого региона при отсутствии несохранённого потребовала бы второго
   лейбла и второго визуального состояния, то есть двух компонентов в одном.

### 2026-09-03 · P6 · CHANGELOG не создан

§9 просит «CHANGELOG / release notes: список редиректов». В репозитории нет
ни CHANGELOG, ни release notes, а завести новый документ ради одного списка —
хуже, чем оставить его там, где он уже есть: полная таблица редиректов лежит
в §1.1 этого плана, источник истины — `frontend/next.config.mjs`.

### 2026-09-03 · P6 · `/admin/pickup` переведён на 308 (закрыто)

Строка в `next.config.mjs` раскомментирована, `app/admin/pickup/page.tsx`
удалён. WU PR-6 не мог этого сделать сам: `next.config.mjs` был у него в
списке «не трогать», а удалять страницу без 308 нельзя — путь перестал бы
вести куда-либо. Сделано владельцем конфига после WU.

Индексные `app/admin/{settings,content}/page.tsx` — не то же самое и
остаются: это корни секций, которых нет в §1.1 как «старых роутов», и
`redirect()` в них намеренный (см. запись про `activePrefix`).

### 2026-09-03 · весь редизайн · DoD п.6 остался невыполненным

Единственный пункт definition of done, не закрытый ни в одном WU: ручная
проверка в браузере на 375 / 768 / 1280 / 1440 с чтением accessibility tree.
Причина одна и техническая: авторизованная админка недостижима из этой сессии.
`/api/auth/me` отдаёт 401; стаб через перехват запросов работает нестабильно
(приложение зависает на `LoadingState`, не запрашивая `/me` вовсе, одинаково
на `/admin`, `/admin/teams` и хабе — то есть дело не в правках); `omp browser
relay` не подключается — расширения в Chrome нет. Дальше первых двух проходов
(дашборд и сайдбар P0/P1, скриншоты в отчётах) продвинуться не удалось.

Что проверено вместо этого по каждому WU: `typecheck`, `lint:design`, полный
`vitest` (1547 зелёных), `next build` без ошибок, `curl -I` всех 31 старых
роутов §1.1 → 308 с правильным `Location`, и behavior-тесты, ассертящие
реальный DOM — включая мобильные карточки через мок `matchMedia` на каждом
T2-экране с > 4 колонками.

Что осталось сделать одним проходом, когда появится сессия: пройти семь
шаблонов на четырёх ширинах, сверить `document.documentElement.scrollWidth <=
innerWidth` и accessible names по accessibility tree. Список экранов —
таблица шаблонов в разделе «Admin kit» в `frontend/DESIGN.md`.
