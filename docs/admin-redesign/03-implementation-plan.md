...
### 2026-09-03 · P6 · `/admin/pickup` не переведён на 308

В `next.config.mjs` строка редиректа лежит закомментированной под маркером
`// PR-6 cleanup`, а `app/admin/pickup/page.tsx` до сих пор делает
`redirect()` в рантайме — ровно то, что DoD п.1 запрещает. Не сделано: WU
PR-6 получил `frontend/next.config.mjs` в списке «не трогать», а без 308
удалять страницу нельзя — путь перестал бы вести куда-либо. Осталось два
действия одним движением для владельца конфига: раскомментировать строку и
удалить `app/admin/pickup/page.tsx`.
