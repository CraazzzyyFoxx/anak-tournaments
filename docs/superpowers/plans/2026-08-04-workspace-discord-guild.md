...
| **P2** `downgrade` cast to `bigint` (max 19 digits) while the schema admitted 20, so a 20-digit id aborted the rollback. Narrowing to 19 digits is **not** sufficient on its own: 19 digits reaches 9.99e18, still over `bigint`. | Narrow `_DISCORD_SNOWFLAKE` to `^\d{17,19}$` **and** bound the cast by `::numeric <= 9223372036854775807`. | `992e66ac`, `25845c0c` |
...
| **P3** Tests asserting a declaration (`model_fields` introspection) two lines from the source. | **Partially accepted.** The `provider_config` one is genuinely subsumed by `test_a_stale_client_guild_is_ignored_not_written` and stays deleted. The `WorkspaceRead` one was **restored** on second review — see the table below. | `25845c0c` |
...
A re-review of the fixes confirmed all four as functionally correct and found no new functional defect, but caught two false statements in the documentation — both mine — plus three smaller items. Verdict was `CHANGES REQUESTED` narrowly; all accepted.

| Finding | Fix |
| --- | --- |
| **P1** The design doc claimed an implausible stored value "ends up `NULL`, which is the fail-open state it effectively already had". Both halves were wrong. The step-1 guard filters *source rows*, so a rejected value falls through to step 2 and may be replaced by a tournament-channel guild — it does not become `NULL` whenever a plausible channel row exists. And per the doc's own asymmetry table, a workspace storing `'999'` with live Boosty was **blocking** every patron, so `NULL` would move it from blocked to passing: a change *away* from its prior state, not a no-op. | Rewrote §4.1 to state the fall-through and why it is deliberate — such a workspace was already broken, so falling through can only fix it or leave it equally broken, whereas `NULL` would silently stop a gate that was enforcing. |
| **P2** Three places claimed an aborted `downgrade` would leave the revision "half-applied". **False for this project:** `migrations/env.py:89-90` wraps `run_migrations` in one transaction and `PostgresqlImpl.transactional_ddl` is `True` (verified against alembic 1.17.2; the offline render emits a single `COMMIT;`), so an abort takes `add_column` with it. The wrong version would send an on-call engineer hunting a half-migrated schema when the correct action is "it rolled back cleanly; fix the value and re-run". | Corrected in the migration docstring, the design doc and this plan. The bound stays — the rollback still would not *complete*. |
| **P3** The numeric bound was two conjuncts (`regex AND ::numeric <= …`). PostgreSQL leaves subexpression evaluation order undefined and the planner may reorder a `WHERE` — the documented counter-example is this exact shape — so the regex is not guaranteed to run before the cast it guards, and a non-digit value could abort the rollback the guard exists to protect. Unreachable today (all three writers produce ≤19 digits), but the guard's whole purpose is surviving an out-of-band value. | Rewritten as `CASE WHEN regex THEN bound ELSE false END`. |
| **P3** Deleting the `WorkspaceRead` pin was **not** an improvement — reversing the earlier reviewer. It left `discord_guild_id` the only `WorkspaceRead` field without one, while `test_workspace_timezone_schema.py:30` and `test_workspace_branding_schema.py:98,104,127` keep character-for-character the same shape. Nothing else fails if the field silently disappears from a model whose public exposure is a deliberate, documented decision; the admin page would just render blank. | Restored, with a comment explaining why it is not the tautology it resembles. |
| **P3** `max_length=32` on `WorkspaceUpdate.discord_guild_id` could never bind — `^\d{17,19}$` already caps at 19 — and both were exported into the public schema, advertising contradictory bounds (`maxLength: 32` beside a ≤19-digit pattern). The same species of dead guard the earlier review had removed from step 1. | Dropped; the `String(32)` column width is noted in the comment instead. |
| **P3** The manifest still advertised `^\d{17,20}$`: `174b1bd1` exported it before `25845c0c` narrowed the pattern, and no CI job diffs `schemas.json`, so the drift would ship silently. | Regenerated. |
