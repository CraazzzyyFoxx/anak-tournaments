...
Числа **заведомо выше** nginx-овских (40 r/s / burst 120 / 100 in-flight): nginx —
основной настраиваемый лимитер, и будь Traefik строже, он молча стал бы
связывающим и начал отдавать реальные 429.

У `rateLimit` `sourceCriterion` не задан сознательно: **его** дефолт — это и есть
клиентский IP соединения, а `forwardedHeaders.trustedIPs` доверяет только
приватным хопам, так что клиентский `X-Forwarded-For` в ключ не попадает.

> **Исправлено 2026-08-15 после инцидента.** Здесь раньше стояло
> `inFlightReq: { amount: 400 }` без `sourceCriterion`, и этот же абзац
> утверждал, что дефолт — клиентский IP. Для `inFlightReq` это **неверно**:
> `pkg/middlewares/inflightreq/inflight_req.go` принудительно ставит
> `RequestHost: true`, если не заданы ни `IPStrategy`, ни `RequestHeaderName`,
> ни `RequestHost`. То есть `amount` был **одним общим пулом одновременных
> запросов на весь `owt.craazzzyyfoxx.me`** — не подстраховка, а глобальный
> потолок конкурентности.
>
> Он сработал: медленный фронтенд (см. §8.6) поднял конкурентность выше 400, и
> Traefik начал отдавать **всем пользователям на всех путях** `429` с телом
> `max connections reached: 400` — строка из `vulcand/oxy` `MaxConnError`, где
> 400 это величина лимита, а не HTTP-код. За 23 минуты 541 отказ, размазанный
> по 100+ разным клиентским IP (максимум 38 на IP) — подпись глобального пула,
> а не персонального лимита.
>
> `depth: 0` обязателен и не является косметикой: парсер отвергает пустую карту
> `ipStrategy: {}` с ошибкой `ipStrategy cannot be a standalone element`, а
> `IPStrategy.Get()` отображает `Depth == 0` без `excludedIPs` в
> `RemoteAddrStrategy` — адрес самого соединения. Проверено на 3.6.2 в песочнице:
> с этим блоком второй запрос с другого IP проходит, без него получает 429.
>
> `amount` снижен с 400 до 300 — теперь это лимит **на IP**, и он должен
> оставаться выше того, что nginx и так разрешает одному адресу
> (`conn_edge 100` + `conn_ws 128` = 228 в худшем случае), чтобы связывающим
> лимитером остался nginx.
...
### 8.5 Ротация логов nginx и Traefik — ✅ применено

Замена stdout-симлинков образа на реальные файлы забрала у nginx ротацию, которую
раньше делал docker json-file. Сервисы приложения ротируются сами (Loguru и
собственная ротация gateway — отсюда `*.log.gz` рядом с их логами), поэтому
первым правилом стал nginx: `/etc/logrotate.d/owt-nginx`, daily, 14 копий,
`copytruncate` (nginx в контейнере держит fd, сигналить с хоста нечем).
...
Traefik пропустили: он пишет `accessLog` в реальный файл и не ротирует его сам.
К 2026-08-15 `/var/log/traefik/access.log` дорос до **7 ГБ** на диске, уже
занятом на 80%. Правило добавлено тогда же — `/etc/logrotate.d/traefik`, daily,
7 копий, `maxsize 500M`.

В отличие от nginx здесь **не** `copytruncate`, а переименование плюс
`systemctl kill -s USR1 traefik.service`: Traefik — юнит systemd, а не
контейнер, поэтому ему **можно** послать сигнал, а по `USR1` он закрывает и
заново открывает свои логи. Проверено на 3.6.2: после `mv` и `USR1` появляется
свежий `access.log`, и запись продолжается в него.

`delaycompress` здесь принципиален: он откладывает сжатие только что
повёрнутого файла на следующий цикл, чтобы многогигабайтная ротация не жгла CPU
на gzip в момент поворота — на этой машине он пришёлся бы на живой трафик.

### 8.6 Фронтенд как узкое место — ✅ исправлено 2026-08-15

Инцидент из §8.1 вскрыл причину уровнем выше лимитеров. Traefik лишь
**показал** насыщение; создавал его фронтенд.

Next standalone запускается как `node server.js` — **один процесс**, то есть
один поток JS независимо от квоты CPU. Все SSR-рендеры сайта стоят в очереди к
этому единственному event loop. Замеры за 23 минуты (1401 с):

| Категория | Запросов | Суммарное время in-flight | Среднее |
|---|---|---|---|
| Страницы (SSR) | 34 520 | **71 211 с (90%)** | 2063 мс |
| API | 14 956 | 6 911 с | 462 мс |
| Статика | 1 028 | 1 238 с | 1204 мс |

В простое страница отдаётся за 100–250 мс; под этой нагрузкой p50 174 мс,
**p90 7.25 с, p99 14.5 с**. По закону Литтла 79 386 с / 1401 с ≈ 57 средней
конкурентности, а на пиках 68–71 rps — выше 400.

Источник объёма — **префетч `next/link` по попаданию во вьюпорт**. Все маршруты
`/tournaments/[id]/*` под `force-dynamic`, поэтому каждый префетч это полный
серверный рендер без кэша, а самые «ссылочные» поверхности рендерят их все
сразу: рейка разделов — девять ссылок, брекет добавляет пару на каждый матч,
шапка с подвалом ещё около шести. Подпись в логах — почти равномерное
распределение по несвязанным маршрутам: `/privacy` 463 обращения, `/terms` 448,
`/users` 444 при **126 уникальных клиентах**, то есть по 12–19 рендеров политики
конфиденциальности на каждого. В логе nginx это видно как ответы по 177 и 266
байт с `ref` страницы брекета — RSC-payload'ы префетча, а не страницы.

Исправлено в репозитории: `frontend/src/components/HoverPrefetchLink.tsx`
переводит префетч с вьюпорта на намерение (hover/focus) и используется в
`TournamentSectionNav`, `BracketView` и `SiteNav`; у ссылок шапки и подвала
префетч выключен совсем (`prefetch={false}`) — это адреса, куда заходят от силы
раз. Проверено в браузере на прод-сборке: три ссылки, видимые 2.5 с, дают ноль
префетч-запросов, наведение на одну — ровно её префетч с заголовком
`Next-Router-Prefetch: 1`.

Ёмкость: `frontend` переведён на `deploy.replicas: 3` с ceiling 1.50 CPU на
реплику вместо одной с 3.00. Реплики объявлены **в compose**, а не через
`PROD_SCALE`, потому что прод выкатывается GitHub-релизом, который зовёт
`docker compose up -d` без `--scale`.
...
Фазы 0 и 1 раскатаны на dd-new 2026-08-06 (§13). Запросы для фазы 2 —
`limit_req` промотирован в метку Loki, `uri`/`remote_addr` живут в теле:

```logql
# по путям: какие эндпоинты упёрлись бы в лимит
sum by (uri) (count_over_time({job="nginx", limit_req="REJECTED_DRY_RUN"} | json [24h]))

# по источникам: отделить реальных пользователей от ботов и своих прогонов
topk(10, sum by (remote_addr) (count_over_time({job="nginx", limit_req="REJECTED_DRY_RUN"} | json [24h])))
```

Собственные нагрузочные прогоны отсеивать по `remote_addr` — публичный IP хоста
хайрпинится обратно и выглядит как обычный клиент.
