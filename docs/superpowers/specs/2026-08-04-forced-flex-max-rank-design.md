...
### D6. `ow_rank_value` сплющивается тем же правилом
...
Решение: `effOwRank = max(ow_rank_value)`, delta = `|effRank − effOwRank|`.
Сравнение эффективного ранга с эффективным OW-рангом, одна честная плашка,
предохранитель остаётся рабочим. Отклонено «отключить проверку в forced-режиме»:
теряется защита от заведомо неверных рангов.
...
`rank_value = effRank`, `ow_rank_value = effOwRank`,
