...
async def assert_no_active_draft_session(session: AsyncSession, tournament_id: int) -> None:
    """Raise a business error when the tournament has an in-flight draft session.

    Changing ``team_formation`` while a draft is in flight would orphan the
    session (SK-O2), so callers invoke this before applying such a change.
    """
