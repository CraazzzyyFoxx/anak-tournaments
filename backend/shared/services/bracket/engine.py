...
def placeholder_seeds(count: int, *, offset: int = 0) -> list[int]:
    """Stand-in seed ids for a bracket whose teams are not known yet.

    Negative, so a caller who slips one past a boundary can never collide with
    a real team id; a caller that persists a placeholder bracket must map every
    negative id back to ``None`` (a TBD slot).
    """
    return [-(offset + index + 1) for index in range(count)]


def placeholder_bracket(
    stage_type: StageType,
    upper_count: int,
    *,
    lower_count: int = 0,
) -> BracketSkeleton:
    """The bracket `generate_bracket` will build once the seeds are known.

    Runs the real generator against `placeholder_seeds`, so a bracket predicted
    (or generated ahead of its seeds) can never drift in shape from the one the
    resolved teams eventually produce -- notably double elimination's negative
    lower-bracket round numbers, and the round count depending on team count
    rather than a stage's independently-set ``max_rounds``.

    ``lower_count`` seeds start in the lower bracket (double elimination only).
    Empty when fewer than two teams reach the upper bracket.
    """
    if upper_count < 2:
        return BracketSkeleton(pairings=[], total_rounds=0)
    if stage_type not in (StageType.SINGLE_ELIMINATION, StageType.DOUBLE_ELIMINATION):
        raise ValueError(f"Unsupported stage type for a placeholder bracket: {stage_type}")

    return generate_bracket(
        stage_type,
        placeholder_seeds(upper_count),
