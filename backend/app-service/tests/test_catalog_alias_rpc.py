...
def test_the_list_operation_publishes_its_filters_as_query_params() -> None:
    ...
    assert op.query is schemas.CatalogAliasMissListQueryParams
...
def test_the_attach_operation_publishes_its_request_body() -> None:
    assert openapi_schemas.OPERATIONS["rpc.app.catalog_aliases.attach"].request is schemas.CatalogAliasAttach
...
def test_attach_requires_entity_type_id_and_alias() -> None:
    ...
    with pytest.raises(ValidationError):
        schemas.CatalogAliasAttach.model_validate({"entity_type": "hero", "entity_id": 1})
    with pytest.raises(ValidationError):
        schemas.CatalogAliasAttach.model_validate({"entity_id": 1, "alias": "Ана"})


def test_attach_rejects_an_unknown_entity_type() -> None:
    with pytest.raises(ValidationError):
        schemas.CatalogAliasAttach.model_validate({"entity_type": "tournament", "entity_id": 1, "alias": "x"})
...
def test_attach_rejects_a_blank_alias(blank: str) -> None:
    ...
    with pytest.raises(ValidationError):
        schemas.CatalogAliasAttach.model_validate({"entity_type": "map", "entity_id": 1, "alias": blank})
...
def test_attach_rejects_an_alias_longer_than_the_column() -> None:
    with pytest.raises(ValidationError):
        schemas.CatalogAliasAttach.model_validate(
            {"entity_type": "hero", "entity_id": 1, "alias": "x" * (schemas.ALIAS_MAX_LENGTH + 1)}
        )
...
def test_a_miss_carries_the_tournament_the_admin_ui_links_to() -> None:
    ...
    fields = schemas.CatalogAliasMissRead.model_fields
