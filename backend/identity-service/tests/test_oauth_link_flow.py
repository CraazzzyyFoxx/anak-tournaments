...
def test_link_custom_origin_issues_ticket_and_never_links(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    it can only mint a ticket. Task 10R fix 1: the issued ticket must carry
    the verified state's guard_hash as its `lg`."""
    _install_fake_provider(monkeypatch, _oauth_info())
    ...
    # redemption will (pending_link_tickets.redeem), against the SAME fake
    # Redis this call already used.
    ticket_payload = asyncio.run(pending_link_tickets.redeem(result.ticket))
...
def test_link_custom_origin_without_guard_hash_never_issues_ticket(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    _install_fake_provider(monkeypatch, _oauth_info())
    link_mock = AsyncMock(side_effect=AssertionError("must not link when guard_hash is missing"))
    monkeypatch.setattr(OAuthService, "link_oauth_to_existing_user", link_mock)
...
def test_link_complete_links_bearer_user_not_ticket(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ticket = asyncio.run(
        pending_link_tickets.issue(_oauth_info(), {"access_token": "provider-access-token"}, guard_hash=guard_hash)
    )

    bearer_user = SimpleNamespace(id=42, username="bearer-owner")
    result = asyncio.run(oauth_flows.link_complete(session=None, user=bearer_user, ticket=ticket, guard=guard))
...
def test_link_complete_redeem_none_raises_error(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(
            oauth_flows.link_complete(session=None, user=SimpleNamespace(id=1), ticket="never-issued", guard=None)
        )
...
def test_link_complete_ticket_redeemed_exactly_once(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = asyncio.run(oauth_flows.link_complete(session=None, user=bearer_user, ticket=ticket, guard=guard))
    assert first["provider"] == "discord"

    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(oauth_flows.link_complete(session=None, user=bearer_user, ticket=ticket, guard=guard))
    assert exc_info.value.status_code == 400
...
def test_link_complete_rejects_missing_guard_even_with_valid_ticket_and_bearer(
    ...
    link_mock = AsyncMock(side_effect=AssertionError("must not link without a matching guard"))
    monkeypatch.setattr(OAuthService, "link_oauth_to_existing_user", link_mock)
    ...
    with pytest.raises(HTTPException) as exc_info:
        asyncio.run(oauth_flows.link_complete(session=None, user=bearer_user, ticket=ticket, guard=None))
...
def test_link_complete_rejects_mismatched_guard_even_with_valid_ticket_and_bearer(
    ...
    identically."""
    link_mock = AsyncMock(side_effect=AssertionError("must not link with a mismatched guard"))
    monkeypatch.setattr(OAuthService, "link_oauth_to_existing_user", link_mock)
...
def test_link_complete_rejects_when_ticket_has_no_guard_hash_at_all(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    link_mock = AsyncMock(side_effect=AssertionError("must not link when the ticket has no guard hash"))
    monkeypatch.setattr(OAuthService, "link_oauth_to_existing_user", link_mock)
