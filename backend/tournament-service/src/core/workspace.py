...
copy of app-service's ``core/workspace.py``; the single source of truth now
lives in ``shared`` since every service's ``models.Tournament`` (etc.) is the
same class object as ``shared.models.*``.
