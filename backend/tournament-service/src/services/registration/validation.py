...
class RegistrationValidationService:
    """The one validator that needs a session; every rule above stays pure."""
    ...
    async def validate_verified_identity(
        ...
        *,
        ...
        # Multi-column projection over one player's verified handles — a row-tuple
        # read, not a CRUD fetch, so it stays here.
        rows = (
