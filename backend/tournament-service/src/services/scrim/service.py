...
async def _ensure_player(session: AsyncSession, user: models.AuthUser) -> models.User:
    ...
    """
    player = await session.scalar(sa.select(models.User).where(models.User.auth_user_id == user.id))
    if player is not None:
        return player
    player = models.User(name=user.username or user.email, auth_user_id=user.id)
    session.add(player)
    await session.flush()
    return player
...
async def _ensure_container(session: AsyncSession, workspace_id: int) -> models.Tournament:
    ...
    *,
    ...
    if source == "copy":
        ...
        for kind in (PickBanKind.MAP, PickBanKind.HERO):
            resolved = await resolve_config_at_level(
                session,
...
async def _load_room(session: AsyncSession, token: str) -> ScrimRoom:
    room = await session.scalar(sa.select(ScrimRoom).where(ScrimRoom.token == token).options(*_ROOM_LOAD))
    if room is None:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Scrim room not found")
    return room


async def get_room_by_token(session: AsyncSession, user: models.AuthUser | None, token: str) -> dict:
    ...
    """
    room = await _load_room(session, token)
    await assert_tournament_viewable(session, user, room.tournament_id)
    return await serialize_room(session, room, user)


async def list_rooms_for_viewer(session: AsyncSession, user: models.AuthUser, workspace_id: int) -> list[dict]:
...
async def serialize_room(session: AsyncSession, room: ScrimRoom, user: models.AuthUser | None) -> dict:
    encounter = room.encounter
    side = await _viewer_side(session, room, user)
...
async def _assert_under_cap(session: AsyncSession, user: models.AuthUser, limit: int) -> None:
...
async def create_room(
    ...
    *,
    ...
    await _assert_under_cap(session, user, config.max_open_rooms_per_user)
    ...
    container = await _ensure_container(session, workspace_id)

    next_order = await session.scalar(
        sa.select(sa.func.coalesce(sa.func.max(models.Stage.order), 0) + 1).where(
            models.Stage.tournament_id == container.id
        )
    )
    stage = models.Stage(
...
async def claim_side(session: AsyncSession, user: models.AuthUser, token: str) -> dict:
    ...
    existing_side = await _viewer_side(session, room, user)
...
async def close_room(session: AsyncSession, user: models.AuthUser, token: str) -> dict:
