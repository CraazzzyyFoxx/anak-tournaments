...
async def _publish_tournament_changed(
    session: AsyncSession, tournament_id: int, reason: TournamentChangedReason
) -> None:
...
async def merge_group_stages(
    ...
    *,
    ...
    await _publish_tournament_changed(session, target_stage.tournament_id, "structure_changed")
    await session.commit()
...
async def _generate_stage_skeleton(
    ...
    if stage.stage_type == enums.StageType.SWISS:
        swiss_standings, swiss_played_pairs, swiss_round = await _get_swiss_generation_context(
