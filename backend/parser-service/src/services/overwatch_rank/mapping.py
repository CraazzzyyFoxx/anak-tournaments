...
The default is an Overwatch 2 SR-aligned scale: divisions are 500 points wide
and each of their 5 tiers spans 100, with tier 5 the bottom and tier 1 the top
of a division. The ladder is anchored at Bronze 5 = 500, so the nine divisions
run 500..4899 with Ultimate 1 open-ended above 4900. The mapping is
configurable at runtime via the ``parser.rank_mapping`` settings key — admin-
provided entries override individual cells of this default. The native
``division``/``tier`` are always stored on the snapshot regardless, so a
mapping miss never loses source data, and a rebase of this table (see
...
DEFAULT_OW2_DIVISION_BASE: Mapping[str, int] = MappingProxyType(
    {
        enums.RankDivision.bronze.value: 500,
        enums.RankDivision.silver.value: 1000,
        enums.RankDivision.gold.value: 1500,
        enums.RankDivision.platinum.value: 2000,
        enums.RankDivision.emerald.value: 2500,
