...
(Linux-only, mirrors `native/moo_core`'s maturin build for the same reason:
no Windows/macOS toolchain assumption).

Do not hand-edit `balance_engine.cpp`/`.hpp`/`pybind11_bindings.cpp` without
