...
class DraftClockService:
    ...
    async def fire_autopick_if_expired(
        ...
        async with session_factory() as session:
            ...
            if draft is None or draft.status != DraftStatus.LIVE.value or draft.current_pick_id is None:
                return False
            pick = await self.picks_repo.get(session, draft.current_pick_id)
    ...
    async def _renewer(self, redis: Redis, token) -> None:
        while True:
    ...
    async def clock_loop(self, session_factory: SessionFactory, redis: Redis, session_id: int) -> None:
        try:
        ...
        renew_task = asyncio.create_task(self._renewer(redis, token))
        pubsub = redis.pubsub()
        await pubsub.subscribe(control_channel(session_id))
        try:
            while not renew_task.done():
                status, expires = await self._read_clock_state(session_factory, session_id)
    ...
    async def draft_clock_supervisor(self, session_factory: SessionFactory, redis: Redis) -> None:
        ...
        while True:
            ...
            try:
                async with session_factory() as session:
                    live_ids = await self.sessions_repo.list_live_ids(session)
                has_live = bool(live_ids)
                for sid in live_ids:
                    task = spawned.get(sid)
                    if task is None or task.done():
                        spawned[sid] = asyncio.create_task(self.clock_loop(session_factory, redis, sid))
            except Exception:  # noqa: BLE001
...
draft_clock_service = DraftClockService()
