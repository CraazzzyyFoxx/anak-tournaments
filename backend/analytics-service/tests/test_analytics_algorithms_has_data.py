...
class AlgorithmHasDataTests(IsolatedAsyncioTestCase):
    async def test_annotates_has_data_for_tournament(self) -> None:
        ...
        with (
            patch.object(analytics_flows.analytics_read_service, "get_algorithms", AsyncMock(return_value=algorithms)),
            patch.object(
            ...
            result = await analytics_flows.flows_service.get_algorithms(session, _params(), tournament_id=7)
    ...
    async def test_has_data_is_none_without_tournament(self) -> None:
        ...
        with (
            patch.object(analytics_flows.analytics_read_service, "get_algorithms", AsyncMock(return_value=algorithms)),
            patch.object(analytics_flows.analytics_read_service, "get_algorithm_ids_with_shift_data", ids_mock),
        ):
            result = await analytics_flows.flows_service.get_algorithms(session, _params())
