"""Pydantic schemas for ML reads and analytics jobs.

Field names, order, defaults, aliases and validators are the wire contract —
RPC response bytes must stay identical. No web-framework dependency.
"""
...
class AnalyticsJobCreate(BaseModel):
    ...
    kind: typing.Literal["compute", "train_ml"] = Field(
        default="compute",
        description=(
            "'compute' (organizer-allowed) runs rating recalc + ML inference. "
            "'train_ml' (superuser only) (re)trains ML boosters."
        ),
    )
    algorithms: list[str] | None = Field(
        default=None,
        description=(
            "For kind=compute: rating algorithm names to recalc (None = all). "
            "For kind=train_ml: model kinds ['performance','shift','standings']."
        ),
    )
