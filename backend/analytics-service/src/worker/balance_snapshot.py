...
__all__ = ("BalanceSnapshotService", "balance_snapshot_service", "register")


class BalanceSnapshotService:
    async def write_balance_snapshot(
        self, session: AsyncSession, event: BalanceExportedEvent
    ...
    async def consume_balance_exported(data: dict, msg: RabbitMessage) -> None:
        async with observe_message_processing(
            ...
            async with db.async_session_maker() as session:
                await balance_snapshot_service.write_balance_snapshot(session, event)
