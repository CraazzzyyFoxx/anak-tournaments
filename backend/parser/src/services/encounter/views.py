...
router = APIRouter(
    prefix="/encounter", tags=[enums.RouteTag.ENCOUNTER], dependencies=[Depends(auth_flows.current_user)]
)


@router.post(path="/bulk")
async def bulk_create_from_challonge(session: AsyncSession = Depends(db.get_async_session)):
