...
async def create_or_ignore_battle_tags(
    session: AsyncSession, player: models.User, in_battle_tags: list[str]
) -> None:
    ...
    for battle_tag in set(maybe_need_add_battle_tags):
        if (
            ...
            try:
                ...
                await service.create_battle_tag(
...
async def create_or_ignore_discords(
    session: AsyncSession, player: models.User, in_discords: list[str]
) -> None:
    ...
    for discord in set(maybe_need_add_discords):
        if (
            discord
            and discord not in discords
            and not await service.get_discord(session, discord)
        ):
            await service.create_discord(session, player, discord=discord)


async def create_or_ignore_twitches(
    session: AsyncSession, player: models.User, in_twitches: list[str]
) -> None:
    ...
    for twitch in set(maybe_need_add_twitches):
        if (
            twitch
            and twitch not in twitches
            and not await service.get_twitch(session, twitch)
        ):
...
async def create(session: AsyncSession, data_in: schemas.UserCSV) -> models.User:
    ...
    if not player_data:
        ...
        await create_or_ignore_battle_tags(
            session, user, [*data_in.smurfs, data_in.battle_tag]
        )
    else:
        ...
        twitch_names: dict[str, models.UserTwitch] = {
            twitch.name: twitch for twitch in user.twitch
        }
        discord_names: dict[str, models.UserDiscord] = {
            discord.name: discord for discord in user.discord
        }
        ...
        if data_in.twitch:
            if data_in.twitch not in twitch_names.keys():
                ...
                await service.update_twitch(
                    session, twitch_names[data_in.twitch], name=data_in.twitch
                )
        if data_in.discord:
            if data_in.discord not in discord_names.keys():
                ...
                await service.update_discord(
                    session, discord_names[data_in.discord], name=data_in.discord
                )
