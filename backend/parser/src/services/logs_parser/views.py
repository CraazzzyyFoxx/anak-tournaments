...
async def process_tournament_logs(tournament_id: int, session=Depends(db.get_async_session)):
    tournament = await tournaments_flows.get(session, tournament_id, [])
    await task_router.broker.publish({"tournament_id": tournament.id}, "process_tournament_logs")
...
async def process_match_logs(tournament_id: int, filename: str):
    async with db.async_session_maker() as session:
        ...
        processor = flows.MatchLogProcessor(tournament, filename.split("/")[-1], decoded_lines)
...
async def process_tournament_logs(tournament_id: int):
    async with db.async_session_maker() as session:
        ...
        for log in logs:
            await task_router.broker.publish({"tournament_id": tournament_id, "filename": log},
        PROCESS_MATCH_LOGS_TOPIC)
