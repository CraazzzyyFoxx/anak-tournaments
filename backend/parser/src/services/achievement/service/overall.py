    ...
    best_standing = (
        sa.select(
            ...
            sa.func.row_number().over(
                ...
                order_by=(
                    sa.case(
                        (models.Standing.buchholz.is_(None), 1),
                        else_=0
                    ).desc()
                )
            ).label("rn")
    ...
    if group_by == "tournament":
        for user_id, tournament_id in user_ids:
            user_achievement = models.AchievementUser(
                user_id=user_id, achievement_id=achievement.id, tournament_id=tournament_id
            )
...
async def calculate_honor_and_glory_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="honor-and-glory", required_value=1, required_position=1, operator_str=">="
    )


async def calculate_two_wins_players_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="two-wins-players", required_value=2, required_position=1, operator_str="=="
    )


async def calculate_three_wins_players_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="three-wins-players", required_value=3, required_position=1, operator_str=">="
    )


async def calculate_sisyphus_and_stone_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="sisyphus-and-stone", required_value=3, required_position=2, operator_str="=="
    )


async def calculate_dahao_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="dahao", required_value=2, required_position=1, operator_str="==", count_by="role"
    )


async def calculate_pathological_sucker_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session, slug="pathological-sucker", required_value=3, required_position=2, operator_str=">=", count_by="role"
    )


async def calculate_lord_of_all_the_elements_achievements(session: AsyncSession) -> None:
    await calculate_wins_achievement(
        session,
...
async def calculate_backyard_cyber_athlete_achievements(session: AsyncSession) -> None:
    ...
    query = (
        sa.select(models.Player.user_id.distinct())
        .join(models.Tournament, models.Tournament.id == models.Player.tournament_id)
        .where(models.Tournament.is_league.is_(True), models.Player.is_substitution.is_(False))
    )
...
async def calculate_best_in_logs(
    ...
    log_value = sa.func.sum(
        sa.case((sa.and_(models.MatchStatistics.name == log_stat_name), models.MatchStatistics.value), else_=0)
    )
    time_value = sa.func.sum(
        sa.case(
            (sa.and_(models.MatchStatistics.name == enums.LogStatsName.HeroTimePlayed), models.MatchStatistics.value),
            else_=0,
        )
    )
...
async def calculate_ill_definitely_survive_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    await calculate_best_in_logs(
        session, "ill-definitely-survive", enums.LogStatsName.Deaths, tournament, order_by="asc"
    )


async def calculate_killer_machine_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    await calculate_best_in_logs(
        session, "killer-machine", enums.LogStatsName.Eliminations, tournament, order_by="desc"
    )


async def calculate_just_shoot_in_the_head_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    await calculate_best_in_logs(
        session, "just-shoot-in-the-head", enums.LogStatsName.CriticalHitAccuracy, tournament, order_by="desc"
    )


async def calculate_poop_forever_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    await calculate_best_in_logs(
        session, "poop_forever", enums.LogStatsName.HeroDamageDealt, tournament, order_by="desc"
    )


async def calculate_one_shot_one_kill_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    await calculate_best_in_logs(
        session, "one-shot-one-kill", enums.LogStatsName.ScopedCriticalHitAccuracy, tournament, order_by="desc"
    )
