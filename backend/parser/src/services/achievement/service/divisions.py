...
async def calculate_i_need_more_power_achievements(session: AsyncSession, tournament: models.Tournament) -> None:
    ...
    logger.info(f"Achievements 'I need more power' for tournament {tournament.name} created successfully")
...
async def calculate_captains_with_5_division_and_above_achievements(
    ...
    achievement_support = await session.scalar(sa.select(models.Achievement).filter_by(slug="support-above-5-division"))
    ...
    if not achievement_dps or not achievement_tank or not achievement_support:
        logger.error("Achievement Captains with 5 division and above not found. Aborting...")
        return
...
async def calculate_div_achievements(
    ...
    PlayerLag = (
        sa.select(
            ...
            .over(partition_by=(models.Player.user_id, models.Player.role), order_by=models.Player.tournament_id)
            .label("prev_div"),
        )
        .where(models.Player.is_substitution.is_(False))
        .order_by(models.Player.user_id, models.Player.role, models.Player.tournament_id)
        .cte("player_lag")
    )
    ...
    for user_id, t_id in rows:
        to_insert.append(models.AchievementUser(user_id=user_id, achievement_id=achievement.id, tournament_id=t_id))
...
async def calculate_my_drill_will_pierce_the_sky_achievements(session: AsyncSession) -> None:
...
async def calculate_im_fine_with_that_achievements(session: AsyncSession) -> None:
    ...
    player_lag = (
        ...
        .join(models.Tournament, models.Tournament.id == models.Player.tournament_id)
        .where(models.Tournament.is_league.is_(False), models.Player.is_substitution.is_(False))
    ...
    player_segments = (
        sa.select(
            ...
            .over(partition_by=player_changed.c.user_id, order_by=player_changed.c.tournament_id)
            .label("segment_id"),
    ...
    logger.info(f"Achievements 'im-fine-with-that' created successfully " f"for {len(user_ids)} user(s).")
